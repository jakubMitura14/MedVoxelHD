...
# def myRobustHd(a, b,  WIDTH,  HEIGHT,  DEPTH):
#     return lltm_cuda.getHausdorffDistance(a[:], b[:],  WIDTH,  HEIGHT,  DEPTH,1.0, torch.ones(1, dtype =bool).to(cuda0) )

# # #mymedianHd(algoOuttD, golddD,  WIDTH,  HEIGHT,  DEPTH)
# # fullResList= lltm_cuda.getHausdorffDistance_FullResList(algoOuttD, golddD, WIDTH,  HEIGHT,  DEPTH  ,1.0, torch.ones(1, dtype =bool) ).cpu().detach().numpy()
# # print("on cpu len")
# # print(len(fullResList))
# # print( fullResList[585692])
# # print(np.max(fullResList))
# # # for i in range(1,len(fullResList)):
# # #     if(fullResList[i]>100):
# # #         print( i)
...
# algoOuttD=torch.from_numpy(f['algoOuttE'][:].astype(bool)).to(cuda0)
# golddD=torch.from_numpy(f['golddE'][:].astype(bool)).to(cuda0)

# # arr= lltm_cuda.getHausdorffDistance_3Dres(algoOuttD,golddD,  WIDTH,  HEIGHT,  DEPTH,1.0, torch.ones(1, dtype =bool).to(cuda0) ).cpu().detach().numpy()
# # print(np.max(arr))

# # print("aaa")
# # print(myRobustHd(algoOuttD,golddD,  WIDTH,  HEIGHT,  DEPTH))
# # print(myRobustHd(algoOuttD,golddD,  DEPTH ,  HEIGHT,WIDTH))
# # print("aaa 2")
...
# ##f.close()
# #arr= lltm_cuda.getHausdorffDistance_3Dres(a, b,  90,80,70 ,1.0, torch.ones(1, dtype =bool).to(cuda0) ).cpu().detach().numpy()
